...
export function RequestReceipt({request}:{request:ReceivedRequest}) {
  ...
  return <section className="request-receipt" aria-labelledby="request-received">
    ...
    <div className="receipt-option"><h3>{t('Ficar por perto','Stay in touch')}</h3><SocialChannels/><a className="live-action" onClick={()=>track('click:community')} href={href('/comunidade')}>{t('Outras formas de acompanhar','More ways to stay connected')}</a></div>
  </section>;
}
