...
export function SocialChannels() {
  ...
  return <div className="live-social-links" role="group" aria-label={t('Segue a música','Follow the music')}>{available.map(([key,label])=><a key={key} href={safeMusicUrl(links[key])!} target="_blank" rel="noopener noreferrer">{label}<span className="sr-only">{t(' (abre noutro separador)',' (opens in a new tab)')}</span> ↗</a>)}</div>;
}
export function EventContext() {
  const {live,loaded,online,t,lang,links}=useSite(), event=live?.session;
  let date='';
  ...
  return <section className="live-context" aria-label={t('O evento','The event')}>
    ...
    {event?.mode==='twitch'&&safeMusicUrl(links.twitch)&&<a href={safeMusicUrl(links.twitch)!} target="_blank" rel="noopener noreferrer">{t('Abrir a transmissão na Twitch','Open the Twitch stream')} ↗</a>}
  </section>;
}
export function RequestReceipt({request}:{request:ReceivedRequest}) {
  const {t,href}=useSite(), heading=useRef<HTMLHeadingElement>(null);
  ...
  return <section className="request-receipt" aria-labelledby="request-received">
    ...
    <div className="receipt-option"><h3>{t('Apoiar','Support')}</h3><p>{t('Pedir músicas é gratuito. Uma tip é opcional e não é necessária para o pedido ser aceite.','Song requests are free. A tip is optional and is not needed for your request to be accepted.')}</p><a className="live-action" href={'/apoio?event='+encodeURIComponent(request.sessionId)+'&request='+encodeURIComponent(request.id)}>{t('Deixar uma tip','Leave a tip')}</a></div>
    {featured?.title&&featured.artist&&safeMusicUrl(featured.url)&&<div className="receipt-option featured-original"><h3>{t('Enquanto esperas…','While you wait…')}</h3><p>{t('Conhece uma música minha','Discover a song of mine')}</p><strong>{featured.title}</strong><span>{featured.artist}</span><a className="live-action" href={safeMusicUrl(featured.url)!} target="_blank" rel="noopener noreferrer">{t('Ouvir a música','Listen to the song')} ↗<span className="sr-only">{t(' (abre noutro separador)',' (opens in a new tab)')}</span></a></div>}
    <div className="receipt-option"><h3>{t('Ficar por perto','Stay in touch')}</h3><SocialChannels/><a className="live-action" href={href('/comunidade')}>{t('Outras formas de acompanhar','More ways to stay connected')}</a></div>
  </section>;
}
