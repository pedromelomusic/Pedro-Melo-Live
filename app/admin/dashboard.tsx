...
<EventPanel key={s.session.id} s={s} act={act} busy={busy}/></TabsContent><TabsContent value="catalog" forceMount className="admin-panel"><section><h2>{t("Repert\u00F3rio deste concerto / Show repertoire")}</h2><p className="hint">{t("Estados e alinhamento pertencem a este concerto. T\u00EDtulo, artista e letra s\u00E3o partilhados pelo repert\u00F3rio geral. / States and setlist belong to this show. Title, artist and lyrics are shared across the catalogue.")}</p><div className="row"><button disabled={s.session.archived || busy} onClick={() => E({ ...blank })}>{t("Adicionar can\u00E7\u00E3o / Add song")}</button><button disabled={s.session.archived || busy} onClick={() => act({ action: 'session_sync' })}>{t("Trazer novas do repert\u00F3rio geral / Add missing catalogue songs")}</button><button onClick={() => download(s.songs, 'pedro-melo-repertoire-' + s.session.id + '.json')}>{t("Exportar repert\u00F3rio / Export repertoire")}</button></div><label htmlFor="search">{t("Procurar t\u00EDtulo ou artista / Search title or artist")}<input id="search" value={search} onChange={e => Q(e.target.value)}/></label>
        ...
        <ul>{s.songs.filter((song: Song) => (song.title + ' ' + song.artist).toLocaleLowerCase().includes(search.toLocaleLowerCase())).map((song: any) => <li key={song.id}><strong>{song.title}</strong><p>{song.artist} · <span className="state-label">{t(stateNames[song.status])}</span></p><p className="hint">{song.lyrics ? (song.lyricsApproved ? t("Letra pronta / Lyrics ready") : t("Letra privada / Private lyrics")) : t("Sem letra / No lyrics yet")}</p><div className="song-actions"><button disabled={busy || s.session.archived} onClick={() => E({ ...song, lyricsApproved: !!song.lyricsApproved })}>{t("Editar / Edit")}</button><button disabled={busy || s.session.archived || song.inSetlist} onClick={() => act({ action: 'setlist_add', id: song.id })}>{t("+ Alinhamento / Setlist")}</button><button disabled={busy || s.session.archived || s.activeSessionId !== s.session.id || song.status === 'playing'} onClick={() => change(song, 'playing')}>{t("Tocar agora / Play now")}</button><NativeSelect aria-label={'Estado de ' + song.title} disabled={busy || !!s.session.archived} value={song.status} onChange={e => change(song, e.target.value)}>{Object.entries(stateNames).map(([value, label]) => <NativeSelectOption disabled={value === 'playing' && s.activeSessionId !== s.session.id} key={value} value={value}>{t(label)}</NativeSelectOption>)}</NativeSelect></div></li>)}</ul></section>
