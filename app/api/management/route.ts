...
export async function GET(r:Request){
  ...
  try{await provision();const d=db(),activeSessionId=await activeId(),id=new URL(r.url).searchParams.get('sessionId')||activeSessionId||'session-initial';
    const [catalog,session,eventSongs,sessions]=await Promise.all([d.prepare('SELECT id,title,artist,revision,genre,decade,language,mood,recommended FROM songs ORDER BY artist COLLATE NOCASE,title COLLATE NOCASE,id').all(),sessionById(id),d.prepare('SELECT song_id AS id,status,in_setlist AS inSetlist,position FROM session_songs WHERE session_id=?').bind(id).all(),d.prepare('SELECT id,name,archived FROM sessions ORDER BY created DESC,id').all()]);
...
export async function POST(r:Request){
  ...
  try{
    ...
    if(b.action==='duplicate')b.newId=crypto.randomUUID();
    ...
    const changed=b.action==='duplicate'?results[0].meta.changes:results.at(-1)?.meta.changes;
    return changed?json({ok:true,...(b.action==='duplicate'?{sessionId:b.newId}:{})}):json({error:'conflict'},409);
