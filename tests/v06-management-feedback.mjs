...
await click('Metadados / capa');assert(text().includes('Sem capa.'));assert(!document.querySelector('a[href^="/api/song-artwork"]'));assert(button('Remover capa').disabled);assert(!text().includes('404'));await click('Fechar');
artStatus=200;await click('Metadados / capa');assert(document.querySelector('a[href^="/api/song-artwork"]'));assert(!button('Remover capa').disabled);await click('Remover capa');assert(text().includes('Sem capa.'));assert(!document.querySelector('a[href^="/api/song-artwork"]'));await click('Fechar');
